...
const downloader = require('image-downloader')
	// قم بالإعلان عنه في الأعلى حتى لا تضطر إلى الإبلاغ عنه مرة أخرى
...
const hastag = str.split(' ').filter(i => i.startsWith('#')).join(', ');
       var callback = () => api.sendMessage({body:`✓هاشتاك: ${hastag}\n✓معرف الفيديو: ${res.data.data.author.unique_id}\n✓أسم الناشر: ${res.data.data.author.nickname}\n-->الوقت: ${res.data.data.digg_count}\n-->وقت المعالجة:${res.data.data.comment_count}`,attachment: fs.createReadStream(__dirname + "/cache/tkvd.mp4")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/tkvd.mp4"),event.messageID);
...
default: api.sendMessage(`==> فيديو ( video/فيديو)
==> صوت (music/audio/صوت)
==> info ( معلومات/info)
==> search ( search/بحث)`, threadID)
}
}catch(e){
